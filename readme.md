...
O benchmark avalia três estratégias de sincronização operando sobre dois protocolos de rede diferentes (WebSocket e TCP Raw) e uma base de dados (PostgreSQL):
...
Crie um arquivo chamado .env na raiz do projeto e adicione a URL de conexão com a base de dados local:
...
Ele contém as métricas de tempo de convergência, mensagens trafegadas, consumo de largura de banda e overhead de metadados em memória para todos os cenários, abrangendo as diferentes estratégias, protocolos e cargas de 1 a 1000 bots simultâneos.